...
export class TestRunnerService {
    ...
    async runTest(testFileName: string): Promise<TestResult> {
        ...
        try {
            ...
            this.logger.log(
                `Test execution completed successfully in ${duration}ms`,
            );
            this.logger.debug(`Test output (first 500 chars): ${output.substring(0, 500)}`);
            ...
            this.logger.debug(`Error output (first 1000 chars): ${output.substring(0, 1000)}`);
    ...
    async runAllTests(): Promise<TestResult> {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            this.logger.error(`All tests failed after ${duration}ms: ${error.message}`);
